...
import { Context, PaginatedComments } from '../types'
...
export default class CommentResolver {
  ...
  @Query(() => [Comment])
  comments(@Arg('postId', () => Int) postId: number, @Ctx() { prisma }: Context) {
  ...
  async paginatedComments(
    ...
    return { comments: posts.slice(0, realLimit), hasMore: limitPlusOne === posts.length }
  }
